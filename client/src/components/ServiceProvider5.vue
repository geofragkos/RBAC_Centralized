  ...
  methods: {
    ...
    onClickItemPerm(key, parsedFirstname, parsedLastname, parsedRole) {
      const payload = {
        ...
        parent: 'Service Provider 5',
      };
      this.showPerm(payload);
    },
    showPerm(payload) {
      const path = 'http://localhost:5001/showperm';
    ...
    getEntities() {
      const path = 'http://localhost:5001/sp5';
      axios.get(path)
        .then((res) => {
    ...
    updateEntity(payload) {
      const path = 'http://localhost:5001/sp5modify';
      axios.put(path, payload)
        .then(() => {
          this.getEntities();
        })
    ...
    revokeRole(payload) {
      const path = 'http://localhost:5001/sp5revoke';
      axios.put(path, payload)
        .then(() => {
          this.getEntities();
        })
