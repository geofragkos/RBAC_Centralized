...
export default {
  ...
  methods: {
    ...
    successPermAlert() {
      ...
      if (this.perm_username === '' || this.perm_device === '' || this.perm_operation === '') {
        this.$swal({
          icon: 'error',
          type: 'success',
          title: 'Error in Search',
        ...
        axios.put(path, payload)
          .then((response) => {
            if (response.data.flag === 'False') {
              ...
              const permAssociation = response.data.entity;
              const parsedRole = response.data.role;
              console.log(permAssociation);
              const payload2 = {
                username: this.perm_device,
              };
              const path2 = 'http://localhost:5001/get_der_info';
              axios.put(path2, payload2)
                .then((response2) => {
                  if (response2.data.flag === 'False') {
                    ...
                    const payload3 = {
                      role: parsedRole,
                      firstName: this.perm_username.split(' ')[0],
                      lastName: this.perm_username.split(' ')[1],
                      parent: permAssociation,
                      model: this.perm_model,
                      operation: this.perm_operation,
                    };
                    const path3 = 'http://localhost:5001/showperm';
                    axios.put(path3, payload3)
                      .then((response3) => {
                        if (response3.data.flag === 'False') {
                          ...
                          || permAssociation2.includes(this.perm_username)) {
                            if (response3.data.answer === 'Yes') {
                              if (this.perm_checkbox_write === false
                              && this.perm_checkbox_read === false) {
                                this.$swal({
                                  icon: 'error',
                                  type: 'success',
                                  title: 'Search Results',
                              ...
                              || this.perm_checkbox_write === true) {
                                if (response3.data.answer_op.includes(this.permissions)) {
                                  ...
                                  this.$swal({
                                    ...
                                    type: 'success',
                                    title: 'Search Results',
                                    text: 'The Access token is not granted. User does not have the requested permissions.',
                                  });
                                }
                              }
                            } else {
    ...
    successAddAlert() {
      if (this.add_firstname === '' || this.add_lastname === '' || this.add_role === 'Choose Role...' || this.add_association === 'Choose Association...') {
        ...
        if (this.add_role === 'DER Owner' && this.enter_der === '') {
          ...
          const path = 'http://localhost:5001/add_user';
          axios.put(path, payload)
            .then((response) => {
    ...
    successDERSearchAlert() {
      ...
      if (this.der_number === '') {
        ...
        const path = 'http://localhost:5001/get_der_info';
        axios.put(path, payload)
          .then((response) => {
            if (response.data.flag === 'False') {
              this.$swal({
    ...
    successAddDERAlert() {
      ...
      if (this.add_der_device === '' || this.add_der_association[0] === 'Choose Association...') {
        ...
        const path = 'http://localhost:5001/add_der_device';
        axios.put(path, payload)
          .then((response) => {
    ...
    successVerifyAlert() {
      ...
      if ((this.verify_name === '') || this.verify_role === 'Choose Role...') {
        ...
        const path = 'http://localhost:5001/verify_utr';
        axios.put(path, payload)
          .then((response) => {
    ...
    successDeleteAlert() {
      ...
      if (this.delete_username === '') {
        ...
        const path = 'http://localhost:5001/delete_user';
        axios.put(path, payload)
          .then((response) => {
            console.log(response.data.sent_verification);
            if (response.data.flag === 'True') {
    ...
    successBlockAlert() {
      ...
      const path = 'http://localhost:5001/find_info';
      axios.put(path, payload)
        .then((response) => {
          this.$swal({
            icon: 'info',
